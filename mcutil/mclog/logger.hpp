...
namespace mclog {
	...
	constexpr int INDENT_DEFAULT_DESC = 2;
	...
	*/
	...
	public:
		...
		* @param force_newline Make newline if the length of stream is larger than
		*                      _max_length
		*/
		void print(LEVEL level, std::stringstream& ss, bool force_newline = true);
		...
		*/
		...
		size_t maxLength() { return this->_max_length; }
	...
	*/
	...
	public:
		...
		McrtLogStream(LEVEL level) :
			_level(level) {}


		McrtLogStream(LEVEL level, const std::string& prefix);
		...
		template <typename T>
		McrtLogStream& operator<<(const T& msg) {
			this->_ss << msg;
			return *this;
		}


		McrtLogStream& operator<<(std::stringstream& ss) {
			this->_ss << ss.str();
			return *this;
		}
