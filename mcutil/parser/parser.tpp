...
namespace mcutil {
    ...
    Argument<T>::Argument(
        ...
        ArgumentInterface(field, desc, is_required, has_range, size),
    ...
    void Argument<T>::find(ArgInput& card) const {
        ...
        // check data length
        if (this->size() > 0 && this->size() != (int)iter->second.size())
    ...
    template <typename T>
    std::string Argument<T>::dataType() const {
    ...
    T stringTo(const std::string& str) {
        ...
        converter >> value;
        if (converter.fail())
            mclog::fatal() << "Type conversion error detected at value '" << str << "'";
        return value;
    }
