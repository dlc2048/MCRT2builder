...
namespace mcutil {
    ...
    void Input::_setMacro() {
        ...
        while (!this->eof()) {
            ...
            if (key.front() == '$' && key.back() == '$') {
                if (list.size() < 2)
                    mclog::fatal() << "Macro statement '" << key << "' needs definition";
                Macro::iterator iter = this->_macro.find(key);
                if (iter != this->_macro.end())
                    mclog::fatal() << "Macro statement '" << key << "' is already defined";
                list.pop_front();
                std::string mac_def = mcutil::join(list);
                mclog::debug() << key << " = " << mac_def;
    ...
    Input::Input(const std::string& file_name, bool psyntax) :
        _psyntax(psyntax), _pos_current(0), _pos_last(0) {
        this->_stream.open(file_name);
        if (!this->_stream)
            mclog::fatal() << "Input file '" << file_name << "' is not found";
        else
            mclog::info() << "Read input file stream '" << file_name << "'";
        this->_setMacro();
        this->init();
    }
            ...
            if (std::regex_match(list[i], match, FIELD_PATTERN)) {  // new field
                if (!field.empty()) {
                    if (args.find(field) != args.end())
                        mclog::fatal() << "Field '" << field << "' is duplicated";
