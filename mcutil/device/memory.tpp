...
namespace mcutil {
    ...
    */
    ...
    template <typename T>
    DeviceVectorHelper<T, typename std::enable_if_t<has_nested_device_memory_type<T>::value>>::DeviceVectorHelper(const std::vector<T>& vector_host_member_dev) {
    ...
    template <typename T>
    DeviceVectorHelper<T, typename std::enable_if_t<has_nested_device_memory_type<T>::value>>::DeviceVectorHelper(T* dev_ptr) {
        this->_deviceptr = dev_ptr;
    }


    template <typename T>
    void DeviceVectorHelper<T, typename std::enable_if_t<has_nested_device_memory_type<T>::value>>::free(size_t size) {
        if (!size)
            return;
    ...
    template <typename T>
    DeviceVectorHelper<T, typename std::enable_if_t<std::is_pointer<T>::value>>::DeviceVectorHelper(const std::vector<T>& vector_host_member_dev) {
        this->_length = vector_host_member_dev.size();
        this->_memoryUsageAppend(mcutil::cudaMemcpyVectorToDevice<T>(vector_host_member_dev, &this->_deviceptr));
    }
    ...
    template <typename T>
    DeviceVectorHelper<T, typename std::enable_if_t<std::is_pointer<T>::value>>::DeviceVectorHelper(T* dev_ptr) {
        this->_deviceptr = dev_ptr;
    }


    template <typename T>
    void  DeviceVectorHelper<T, typename std::enable_if_t<std::is_pointer<T>::value>>::free(size_t size) {
