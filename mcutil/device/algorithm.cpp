...
namespace mcutil {
    ...
    AliasTableEGS::AliasTableEGS(
    ...
    ) : _dim(make_int2(0, 0)), _xdata(xs_array), _fdata(fs_array) {
        ...
        this->_dim.y = (int)_xdata.size() - 1;
        this->_dim.x = (int)(_fdata.size() / _xdata.size());

        size_t dimy = (size_t)this->_dim.y;
        size_t dimx = (size_t)this->_dim.x;

        for (size_t i = 0; i < dimx; ++i) {
            std::vector<double> aux(dimy, 0.5);
            for (size_t j = 0; j < dimy; ++j) {
                aux[j] *= (_fdata[i * (dimy + 1) + j + 1] +
                    _fdata[i * (dimy + 1) + j]);
                aux[j] *= (_xdata[j + 1] - _xdata[j]);
            }
            AliasTable segment(dimy, &aux[0]);
    ...
    AliasTableEGS::AliasTableEGS(
        ...
        this->_dim.y = xs_dim;
        this->_dim.x = (int)(_xdata.size() / (xs_dim + 1));
