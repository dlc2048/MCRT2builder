...
#define M_MacroXSAoSMapperVector(type, cbegin, cend, member_alias) {                  \
    std::vector<type> member_list;                                                    \
    for (auto iter = cbegin; iter != cend; ++iter) {                                  \
        member_list.insert(member_list.end(),                                         \
                           iter->member_alias.begin(),                                \
                           iter->member_alias.end()                                   \
        );                                                                            \
    }                                                                                 \
    this->_deviceptr_lists.push_back(DeviceVectorHelper(member_list).devicePtr());    \
}
...
namespace mcutil {
    ...
    constexpr size_t MEMSIZE_KIB = 1 << 10;
    constexpr size_t MEMSIZE_MIB = MEMSIZE_KIB * MEMSIZE_KIB;
    constexpr size_t MEMSIZE_GIB = MEMSIZE_MIB * MEMSIZE_KIB;
    ...
    */
    ...
    public:
        ...
        const std::vector<CUdeviceptr>& deviceptrLists() const { return this->_deviceptr_lists; }
